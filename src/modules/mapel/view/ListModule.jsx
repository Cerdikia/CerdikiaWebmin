...
export default function ListModule() {
  ...
  // Use refs to prevent multiple redirects and fetches
  // const authCheckDone = useRef(false)
  // const dataFetchDone = useRef(false)
  // Use a single initialization flag
  ...
  const fetchData = async () => {
    ...
    // if (dataFetchDone.current) return
    try {
      setLoading(true)
  ...
  // Only check authorization for teachers
  // Load teacher subjects if user is a teacher
  const checkAuthorization = useCallback(() => {
    if (userData?.role === "guru") {
      const guruMapel = JSON.parse(localStorage.getItem("guru_mapel") || "[]")
      setTeacherSubjects(guruMapel)

      // Check if current mapel is in teacher's subjects
      // const isAuthorized = guruMapel.some(
      //   (subject) => subject.id_mapel === Number.parseInt(storedId),
      // )
      // const isAuthorized = guruMapel.some(
      ...
      // if (!isAuthorized) {
      if (!hasAccess) {
        // Redirect to mapel page if teacher is not authorized for this subject
        // navigate("/guru")
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      //   }
      //   // }, [storedId, userData])
      // }, [id, userData, navigate]) // Only depend on these values
